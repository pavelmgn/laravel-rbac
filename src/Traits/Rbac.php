...
namespace Pavelmgn\LaravelRbac\Traits;

trait Rbac
{
	public function roles()
	{
		return $this->belongsToMany('Pavelmgn\LaravelRbac\Model\Role')->with('permissions')->withTimestamps();
	}
	...
	public function canDo($operation)
	{
		...
		if ( ! isset($permissions[$this->id]) ) {
			...
			foreach ($roles as $role) {
				$permissions[$this->id] =
					array_merge($permissions[$this->id], $role->permissions->pluck('slug')->toArray());
			}
